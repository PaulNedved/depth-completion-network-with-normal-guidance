...
class KBNetEncoder(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 input_channels_depth=8,
                 input_channels_normal=24,
                 ...
                 resolutions_backprojection=[0, 1, 2, 3],  # 0 1 2 3
                 weight_initializer='kaiming_uniform',
                 activation_func='leaky_relu'):
        super(KBNetEncoder, self).__init__()

        self.resolutions_backprojection = resolutions_backprojection  # 0 1 2 3
        input_channels_normal = 24
        n_filters_normal = n_filters_image
        n_convolutions_normal = [1, 1, 1, 1, 1]
        ...
        if n in resolutions_backprojection:
            ...
            self.conv0_normal = net_utils.Conv2d(
                in_channels=input_channels_normal,
                out_channels=n_filters_normal[n],
                kernel_size=3,
                stride=1,
                weight_initializer=weight_initializer,
                activation_func=activation_func)
            ...
            in_channels_normal = n_filters_normal[n]
            in_channels_fused = in_channels_image + in_channels_normal + 6
        ...
        in_channels_image = n_filters_image[n - 1]  # 48
        in_channels_depth = n_filters_depth[n - 1]  # 16
        in_channels_normal = n_filters_normal[n - 1]  # 48

        if n in resolutions_backprojection:

            if n - 1 in resolutions_backprojection:
                in_channels_fused = 150
            else:
        ...
        in_channels_normal = n_filters_normal[n - 1]

        if n in resolutions_backprojection:

            if n - 1 in resolutions_backprojection:
                in_channels_fused = 96 * 3 + 6
            else:
                in_channels_fused = n_filters_image[n - 1]
        ...
        if n in resolutions_backprojection:  # yes

            if n - 1 in resolutions_backprojection:
                in_channels_fused = 192 * 3 + 6
            else:
                in_channels_fused = n_filters_image[n - 1]
        ...
        if n in resolutions_backprojection:  # no
            ...
            self.conv5_normal = net_utils.VGGNetBlock(
                in_channels=in_channels_normal,
                out_channels=n_filters_normal[n],
                n_conv=n_convolutions_normal[n],
                stride=2,
                weight_initializer=weight_initializer,
                activation_func=activation_func)

    def forward(self, image, depth, normal, intrinsics):
        ...
        if 0 in self.resolutions_backprojection:
            ...
            conv0_normal = self.conv0_normal(normal)

            # Calibrated backprojection
            conv1_image, conv1_depth, conv1_normal, conv1_fused = self.calibrated_backprojection1(
                image=conv0_image,
                depth=conv0_depth,
                normal=conv0_normal,
                coordinates=coordinates0,
                fused=None)
            skips1 = [conv1_fused, conv1_depth, conv1_normal]
            # print('encoder_info:', conv0_image.size(), conv0_depth.size(), conv0_normal.size())
            # print('Calibrated backprojection_info:', conv1_image.size(), conv1_depth.size(), conv1_normal.size(), conv1_fused.size())
        else:
        ...
        if 1 in self.resolutions_backprojection:
            ...
            conv2_image, conv2_depth, conv2_normal, conv2_fused = self.calibrated_backprojection2(
                image=conv1_image,
                depth=conv1_depth,
                normal=conv1_normal,
            ...
            skips2 = [conv2_fused, conv2_depth, conv2_normal]
        else:
        ...
        if 2 in self.resolutions_backprojection:
            ...
            # Calibrated backprojection
            conv3_image, conv3_depth, conv3_normal, conv3_fused = self.calibrated_backprojection3(
                image=conv2_image,
                depth=conv2_depth,
                normal=conv2_normal,
            ...
            skips3 = [conv3_fused, conv3_depth, conv3_normal]
        else:
        ...
        if 3 in self.resolutions_backprojection:
            ...
            # Calibrated backprojection
            conv4_image, conv4_depth, conv4_normal, conv4_fused = self.calibrated_backprojection4(
                image=conv3_image,
                depth=conv3_depth,
                normal=conv3_normal,
            ...
            skips4 = [conv4_fused, conv4_depth, conv4_normal]
        else:
        ...
        if 4 in self.resolutions_backprojection:
            ...
            conv5_normal = self.conv5_normal(conv4_normal)
            conv5_fused = None

            skips5 = [conv5_image, conv5_depth, conv5_normal]

        # print('skips:', len(skips1), len(skips2), len(skips3), len(skips4), len(skips5), self.resolutions_backprojection)
        # Store as skip connection
        layers.append(torch.cat(skips5, dim=1))

        # layer[-1] = skips5 = [conv5_image, conv5_depth, conv5_normal]
        # layers[0:-1] = skip 1,2,3,4
        # skips1 = [conv1_fused, conv1_depth, conv1_normal] 同 234
...
class MultiScaleDecoder(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 output_channels=4,
                 n_scale=4,  # 1
                 n_filters=[256, 128, 64, 32, 16],  # [256, 128, 128, 64, 12]
                 n_skips=[896, 448, 224, 112, 0],
                 weight_initializer='kaiming_uniform',
                 activation_func='leaky_relu',
                 output_func='linear',  # output_func='linear',
                 use_batch_norm=False,
                 deconv_type='transpose'):  # deconv_type='up',
        super(MultiScaleDecoder, self).__init__()

        output_channels_normal = 3
        n_filters_normal = 3 * n_filters
        ...
        if 'upsample' in self.output_func and self.n_scale < 2:  # no, output_func = linear
            self.n_scale = 2

        # Resolution 1/32 -> 1/16
        # n_skips = [896, 448, 224, 112, 0],  depth normal image 通道数相加
        # n_filters = [256, 128, 128, 64, 12], 给定
        # n_filters_normal = 3 * n_filters
        in_channels, in_channels_normal, skip_channels, out_channels, out_channels_normal = [
            input_channels, input_channels, n_skips[0], n_filters[0], n_filters_normal[0]
        ]
        ...
        # Resolution 1/16 -> 1/8
        in_channels, in_channels_normal, skip_channels, out_channels, out_channels_normal = [
            n_filters[0], n_filters_normal[0], n_skips[1], n_filters[1], n_filters_normal[1]
        ]
        ...
        # Resolution 1/8 -> 1/4
        in_channels, in_channels_normal, skip_channels, out_channels, out_channels_normal = [
            n_filters[1], n_filters_normal[1], n_skips[2], n_filters[2], n_filters_normal[2]
        ]
        if self.n_scale > 3:  # no
        ...
        # Resolution 1/4 -> 1/2
        in_channels, in_channels_normal, skip_channels, out_channels, out_channels_normal = [
            n_filters[2], n_filters_normal[2], n_skips[3], n_filters[3], n_filters_normal[3]
        ]
        if self.n_scale > 2:
            skip_channels = skip_channels + output_channels
        ...
        # Resolution 1/2 -> 1/1
        if 'upsample' not in self.output_func:  # yes
            in_channels, in_channels_normal, skip_channels, out_channels, out_channels_normal = [
                n_filters[3], n_filters_normal[3], n_skips[4], n_filters[4], n_filters_normal[4]
            ]
            if self.n_scale > 1:
                skip_channels = skip_channels + output_channels
    ...
    def forward(self, x, skips):  # x = skip5, skips = skip1234
        layers = [x]  # skip5
        # layers_normal = [x]  # skip5
        outputs = []
        # outputs_normal = []

        # Resolution 1/32 -> 1/16
        n = len(skips) - 1  # 4-1=3
        print('skips_size:', skips[n].size())
        layers.append(self.deconv4(layers[-1], skips[n]))
        # layers_normal.append(self.deconv4_normal(layers_normal[-1], skips[n]))

        # Resolution 1/16 -> 1/8
        n = n - 1  # 3-1
        layers.append(self.deconv3(layers[-1], skips[n]))
        # layers_normal.append(self.deconv3_normal(layers_normal[-1], skips[n]))

        if self.n_scale > 3:  # no  n_scale = 1
            output3 = self.output3(layers[-1])
            outputs.append(output3)
            # outputs_normal.append(output3_normal)
        ...
        # layers_normal.append(self.deconv2_normal(layers_normal[-1], skip))

        if self.n_scale > 2:  # no
        ...
        # layers_normal.append(self.deconv1_normal(layers_normal[-1], skip))
        ...
        if 'upsample' in self.output_func:  # no
            output0 = upsample_output1
        else:
            if self.n_scale > 1:  # n_scale = 1
...
class PoseDecoder(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # print('conv_output.shape', conv_output.shape)  ([8, 6, 3, 6])
        pose_mean = torch.mean(conv_output, [2, 3])
        # print('pose_mean.shape', pose_mean.shape)  ([8, 6])
        dof = 0.01 * pose_mean
        # pose matrix函数功能：将6自由度参数转换为变换矩阵
        # input：Nx6      output：N x 4 x 4 homogeneous transformation matrix
...
class SparseToDensePool_for_normal_contain_pooling(torch.nn.Module):
    ...
    def forward(self, depth, normal, intrinsics):
        ...
        for min_kernel_size in self.min_pool_sizes:
            time_min = datetime.datetime.now()
            output_normal = new_max_pooling(depth, normal, intrinsics,
                                            min_kernel_size,
                                            1)
            pool_pyramid.append(output_normal)

        for max_kernel_size in self.max_pool_sizes:
            time_max = datetime.datetime.now()
            output_normal = new_max_pooling(depth,
                                            normal,
                                            intrinsics,
                                            max_kernel_size,
                                            0)
        ...
        pool_convs = torch.cat([pool_convs, normal], dim=1)

        return self.conv(pool_convs)


def new_max_pooling(depth, normal, intrinsics, kernel_size, min_or_max_lable):
    # min_or_max_lable ==1 表示取夹角最小值，即 min——pool

    n_batch, _, n_height, n_weight = depth.shape
    output_normal = normal
    device = depth.device

    if min_or_max_lable == 1:  # 取夹角最小
        for batch in range(n_batch):
            inverse_k = torch.inverse(intrinsics[batch, :, :])
            time_batch = datetime.datetime.now()
            flag = 0
            for i in range(n_height):
                for j in range(n_weight):
                    a = torch.tensor([j, i, 1], device=device)
                    if i - kernel_size // 2 >= 0 and i + kernel_size // 2 + 1 < n_height \
                            and j - kernel_size // 2 >= 0 and j + kernel_size // 2 + 1 < n_weight:
                        non_zero = torch.zeros((kernel_size * kernel_size, 3), device=device)
                        xyz_list = []
                        count = 0
                        for m in range(i - kernel_size // 2, i + kernel_size // 2 + 1):
                            for n in range(j - kernel_size // 2, j + kernel_size // 2 + 1):
                                if depth[batch, 0, m, n] != 0:
                                    non_zero[count, :] = (normal[batch, :, m, n])
                                    count += 1
                                    z = depth[batch, 0, m, n]
                                    a = a * z
                                    xyz = (torch.matmul(inverse_k, a.T)).T
                                    xyz = torch.nn.functional.normalize(xyz, dim=0)
                                    xyz_list.append(torch.matmul(xyz, (normal[batch, :, m, n].reshape(3, 1))))

                        if len(xyz_list) >= 1:
                            output_normal[batch, :, i, j] = non_zero[xyz_list.index(max(xyz_list)), :]
    else:  # 内容与上述重复，取夹角最大
        for batch in range(n_batch):
            inverse_k = torch.inverse(intrinsics[batch, :, :])
                        ...
                        for m in range(i - kernel_size // 2, i + kernel_size // 2 + 1):
                            for n in range(j - kernel_size // 2, j + kernel_size // 2 + 1):
                                if depth[batch, 0, m, n] != 0:
                                    ...
                                    count += 1
                                    z = depth[batch, 0, m, n]
                                    a = torch.tensor([j * z, i * z, z], device=device)
                                    xyz = (torch.matmul(inverse_k, a.T)).T
                                    xyz = torch.nn.functional.normalize(xyz, dim=0).to(device)
                                    xyz_list.append(torch.matmul(xyz, (normal[batch, :, m, n].reshape(3, 1))))
                        if len(xyz_list) >= 1:
                            output_normal[batch, :, i, j] = non_zero[xyz_list.index(min(xyz_list)), :]
...
class SparseToDensePool_for_normal(torch.nn.Module):
    ...
    def __init__(self,
        ...
        for n in range(n_convolution):  # n_convolution=3   三层1*1卷积
            ...
            # Set new input channels as output channels
            in_channels = n_filter  # 24

        self.pool_convs = torch.nn.Sequential(*pool_convs)

        in_channels = n_filter + input_channels  # 3*3卷积输入通道数  24 + 21
    ...
    def forward(self, depth, normal, intrinsics):
        ...
        # Input depth
        n_batch, _, n_height, n_width = normal.shape  # N x 21 x H x W

        # Learn weights for different kernel sizes, and near and far structures
        pool_convs = self.pool_convs(normal)
