...
class KBNetEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        input_channels_normal = 24
        n_filters_normal = n_filters_image  # [48, 96, 192, 384, 384]
        ...
        if n in resolutions_backprojection:
            ...
            in_channels_fused = in_channels_image + 3*in_channels_depth + in_channels_normal
        ...
        if n in resolutions_backprojection:
            ...
            if n - 1 in resolutions_backprojection:
                in_channels_fused = n_filters_image[n-1] + n_filters_fused[n-1] + in_channels_depth*3 + in_channels_normal
            else:
...
class MultiScaleDecoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.deconv4_normal = net_utils.DecoderBlock(
            in_channels_normal,
            skip_channels,
            out_channels_normal,
            weight_initializer=weight_initializer,
        ...
        self.deconv3_normal = net_utils.DecoderBlock(
            in_channels_normal,
            skip_channels,
            out_channels_normal,
            weight_initializer=weight_initializer,
        ...
        self.deconv2_normal = net_utils.DecoderBlock(
            in_channels_normal,
            skip_channels,
            out_channels_normal,
            weight_initializer=weight_initializer,
        ...
        self.deconv1_normal = net_utils.DecoderBlock(
            in_channels_normal,
            skip_channels,
            out_channels_normal,
            weight_initializer=weight_initializer,
        ...
        if 'upsample' not in self.output_func:      # yes
            ...
            self.deconv0_normal = net_utils.DecoderBlock(
                in_channels_normal,
                skip_channels,
                out_channels_normal,
                weight_initializer=weight_initializer,
            ...
            self.output0_normal = net_utils.Conv2d(
                out_channels_normal,
        ...
        if 'upsample' in self.output_func:   # no
            ...
            if self.n_scale > 1:   # n_scale = 1
                ...
                if n == 0:
                    ...
                    layers_normal.append(self.deconv0_normal(layers_normal[-1]))

            output0 = self.output0(layers[-1])
            output0_normal = self.output0_normal(layers_normal[-1])

        outputs.append(output0)
        outputs_normal.append(output0_normal)

        return outputs, outputs_normal
...
class SparseToDensePool_for_normal(torch.nn.Module):
    ...
    def forward(self, depth, normal, intrinsics):
        ...
        points = points.permute(0, 2, 1)  # N x (H x W) x 3

        normal = normal.permute(0, 2, 3, 1).contiguous().view(n_batch, -1, 3)   # N x H x W x 3,  N x (H x W) x 3

        direction = torch.zeros(n_batch, hw).cuda()
        direction0 = torch.zeros(hw).cuda()
        zero_to_compare = torch.zeros(3,1).cuda()
        for i in range(n_batch):
            points_i = points[i, :, :].view(-1, 3)
            normal_i = normal[i, :, :].view(-1, 3)
            for j in range(hw):
                try:
                    xzy = (points_i[j,:].view(1,3)).cuda()
                    nor = (normal_i[j,:].view(3,1)).cuda()
                    if (nor.equal(zero_to_compare)):
                        direction0[j] = 999
                    else:
                        cos = torch.matmul(xzy,nor)
                        dir = math.degrees(math.acos(cos))   # 这一步是否保留？？？？
                        direction0[j] = dir
                except:
                    direction0[j] = 180
            
            direction[i, :] = direction0

        direction = direction.view(n_batch, 1, n_height, n_width)   # N x 1 x H x W
        ...
        for pool_nor in (self.min_pools):
            ...
            z_pool, idx = pool_nor(direction*(-1))
        ...
        for pool in self.max_pools:
            normal_new = torch.zeros(n_batch,hw).cuda()
            z_pool, idx = pool(torch.where(direction == 999, -1 * torch.zeros_like(direction), direction ))
            # z_pool, idx = pool(direction)
            indices_dim = -idx.view(4,-1)
            for batch in range(4):
                for i in range(hw):
                    normal_new[batch, i]=normal_t[batch, indices_dim[batch, i]]
...
def max_pooling(dir, normal, kernel_size, stride, padding):
    n_batch, _, n_height, n_width = normal.shape   # N x 3 x H x W 
    nor_new = torch.zero(n_batch, 3, n_height, n_width)
    for n in range(n_batch):
        data_nor = normal[n, :, :, :].view(3, n_height, n_batch)
        data = dir[n, :, :, :].view(n_height, n_width)
        for i in range(0, n_height, stride):
            for j in range(0, n_width, stride):
                x = data[i:i+kernel_size, j:j+kernel_size]
                _, idx = x.max()
                a, b = idx
                data_nor[:, i, j] = data_nor[:, i+a, j+b]
        nor_new[n, :, :, :] = data_nor[:, i, j].view(1, 1, n_height, n_width)
