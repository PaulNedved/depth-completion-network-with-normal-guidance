...
def normal_loss_func(image, depth, normal, intrinsics):
    ...
    # depth.shape   torch.Size([8, 1, 320, 768])
    # normal.shape  torch.Size([8, 3, 320, 768])

    # Create homogeneous coordinates [u, v, 1]
    xy_h = net_utils.meshgrid(n_batch, n_height-1, n_width-1, device=depth.device, homogeneous=True)
    xy_h = xy_h.view(n_batch, 3, -1)
    # X(p) = k^-1 * p     shape: N x 3 x (H-1 x W-1) 244,673
    x_p = torch.matmul(torch.inverse(intrinsics), xy_h).permute(0,2,1).contiguous() 
    
    # 1 下临近点， 2 右临近点
    xy_h1, xy_h2 = net_utils.meshgrid_new(n_batch, n_height, n_width, device=depth.device, homogeneous=True)
    xy_h1 = xy_h1.view(n_batch, 3, -1)
    xy_h2 = xy_h2.view(n_batch, 3, -1)

    # q1 下临近点， q2 右临近点
    x_q1 = torch.matmul(torch.inverse(intrinsics), xy_h1).permute(0,2,1)
    x_q2 = torch.matmul(torch.inverse(intrinsics), xy_h2).permute(0,2,1)
