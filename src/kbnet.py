...
def train(train_image_path,
    ...
    # surface_normal_paths = 'training/kitti/kitti_train_normal-clean.txt'
    # train_normal_paths = data_utils.read_paths(surface_normal_paths)
    ...
    # assert len(train_normal_paths) == n_train_sample
    assert len(train_dense_normal_paths) == n_train_sample
    ...
    # 输入normal.shape = 7*3*245760
    # 在data.utils中进行了reshape  21*3*320*768
    ...
    # not sure
    # val_normal_path = 'validation/kitti/kitti_val_normal.txt'
    val_dense_normal_path = 'validation/kitti/kitti_val_dense_normal.txt'
    ...
    if validation_available:
        ...
        val_normal_paths = data_utils.read_paths(val_dense_normal_path)
        ...
        assert len(val_normal_paths) == n_val_sample
    ...
    depth_model = KBNetModel(
        ...
        n_filters_encoder_normal=n_filters_encoder_image,
    ...
    # parameters()会返回一个生成器（迭代器）,然后将其迭代打印出来：
    ...
    augmentation_probability = augmentation_probabilities[0]  # augmentation_probabilities = [1. 0.5, 0.25]
    ...
    for epoch in range(1, learning_schedule[-1] + 1):
        ...
        # augmentation_schedule = [50, 55, 60],  augmentation_schedule_pos = 0
        ...
        # train_dataloader： image0, image1, image2, sparse_depth0, normal0, intrinsics
        for inputs in train_dataloader:
            ...
            [normal0] = train_transforms.transform(
            ...
            output_depth0, output_normal = depth_model.forward(
                image=image0,
                sparse_depth=sparse_depth0,
            ...
            if train_step % 100 == 0:
                print('now time is:', time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime()))
                print(train_step)

            if (train_step % n_checkpoint) == 0:
                time_elapse = (time.time() - time_start) / 3600  # 转为小时
                ...
                if train_step >= validation_start_step and validation_available:
                    ...
                    print('Switch to validation mode')
                    depth_model.eval()
...
def validate(depth_model,
    ...
    for idx, (inputs, ground_truth) in enumerate(zip(dataloader, ground_truths)):
        ...
        image, sparse_depth, dense_normal, intrinsics = inputs
...
def run(image_path,
    ...
    log_path = os.path.join(checkpoint_path, 'results.txt')
    log_net_path = os.path.join(checkpoint_path, 'net.txt')
    ...
    dense_normal_paths = data_utils.read_paths(dense_normal_path)
    ...
    dataloader = torch.utils.data.DataLoader(
        datasets.KBNetInferenceDataset(
            ...
            normal_paths=dense_normal_paths),
        batch_size=1,
        shuffle=False,
        num_workers=1,
        drop_last=False)
    ...
    # print('sparse_to_dense_pool:', depth_model.sparse_to_dense_pool)
    # print('sparse_to_dense_pool_normal:', depth_model.sparse_to_dense_pool_normal)
    # print('encoder:', depth_model.encoder)
    # print('decoder:', depth_model.decoder)

    # for i, child in enumerate(depth_model.encoder.module.children()):
    #     print('element_{}: \n {}'.format(i, child))
    '''
    Log input paths
    '''
    ...
    for idx, (inputs, ground_truth) in enumerate(zip(dataloader, ground_truths)):
        ...
        # print('initial_size \n:', image.size(), sparse_depth.size(), dense_normal.size(), intrinsics.size(), validity_map_depth.size()) 
        ...
        if save_outputs:
            ...
            output_normals.append(output_normal)
        ...
        if ground_truth_available:
            ...
            output_depth = output_depth[mask]
            ground_truth = ground_truth[mask]
            # output_normal = output_normal[mask]
    ...
    if save_outputs:
        ...
        outputs = zip(images, output_depths, sparse_depths, ground_truths, output_normals)
        ...
        output_normal_dirpath = os.path.join(output_path, 'output_normal')
        ...
        for idx, (image, output_depth, sparse_depth, ground_truth, output_normal) in enumerate(outputs):

            if keep_input_filenames:
                filename = os.path.basename(image_paths[idx])
                filename_normal = os.path.basename(image_paths[idx])
            else:
                filename = '{:010d}.png'.format(idx)
                filename_normal = '{:010d}.npy'.format(idx)
            ...
            output_normal_path = os.path.join(output_normal_dirpath, filename_normal)
            data_utils.save_normal(output_normal, output_normal_path)
...
def log_system_settings(log_path,
    ...
    log('Hardware settings:', log_path, to_console=False)
    log('device={}'.format(device.type), log_path, to_console=False)
    log('n_thread={}'.format(n_thread), log_path, to_console=False)
    log('', log_path)
