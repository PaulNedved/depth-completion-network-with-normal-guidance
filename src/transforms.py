...
class Transforms(object):
    ...
    def __init__(self,
        ...
        Arg(s):
            ...
            random_remove_points : list[float] 在范围地图中移除的点百分比
                percentage of points to remove in range map
            random_noise_type : str
                type of noise to add: gaussian, uniform 要添加的噪声类型:高斯、均匀
            random_noise_spread : float
                if gaussian, then standard deviation; if uniform, then min-max range
                如果是高斯分布，则为标准差;如果均匀分布，则最小-最大值范围
        '''

        # Image normalization
        self.normalized_image_range = normalized_image_range # [0, 1]
        ...
        self.remove_points_range = random_remove_points # [0.60, 0.70]
    ...
    def transform(self,
        ...
        if self.do_random_horizontal_flip:   # None  不进。
            ...
            normal_arr = self.horizontal_flip(
                normal_arr,
                do_horizontal_flip)
        ...
        if self.do_random_vertical_flip:  # no
        ...
        if self.do_random_remove_points:   # yes
            ...
            remove_points_min, remove_points_max = self.remove_points_range  # 0.6, 0.7
    ...
    def horizontal_flip(self, images_arr, do_horizontal_flip):
        '''
        Perform horizontal flip on each sample 水平翻转
