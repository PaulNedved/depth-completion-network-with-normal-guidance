...
def load_normal(path):
    z = np.load(path, encoding="latin1", allow_pickle=True)
    z = z.reshape(21, 320, 768)
    return z


def load_test_normal(path):
    z = np.load(path, encoding="latin1", allow_pickle=True)
    z = z.reshape(21, 352, 1216)
    return z
...
def save_normal(z, path):
    '''
    Saves a normal map to a numpy npy file
