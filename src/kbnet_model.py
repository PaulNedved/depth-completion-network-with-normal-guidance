...
def pca_reduce(img_tensor):
    N, D, H, W = img_tensor.size()
    flattened_img = img_tensor.view(N, D, -1).permute(0, 2, 1).contiguous().view(-1, D)
    pca = PCA(n_components=1)
    reduced_img = pca.fit_transform(flattened_img.cpu().detach().numpy())
    reduced_img = torch.tensor(reduced_img, dtype=torch.float32).view(N, H, W)
    # print('D:', D)
    return reduced_img
...
class KBNetModel(object):
    ...
    def __init__(self,
        ...
        self.sparse_to_dense_pool_normal = networks.SparseToDensePool_for_normal(
            input_channels=21,
            min_pool_sizes=min_pool_sizes_sparse_to_dense_pool,
            max_pool_sizes=max_pool_sizes_sparse_to_dense_pool,
            n_convolution=n_convolution_sparse_to_dense_pool,
        ...
        input_channels_depth = n_filter_sparse_to_dense_pool  # 8

        n_filters_encoder = [
            i + z + j
            for i, z, j in zip(n_filters_encoder_image, n_filters_encoder_depth, n_filters_encoder_normal)
        ]
        ...
        self.encoder = networks.KBNetEncoder(
            ...
            input_channels_depth=input_channels_depth,  # 8 从sparse to dense出来的通道数
        ...
        self.decoder = networks.MultiScaleDecoder(
            input_channels=896,  # 384+384+128
            output_channels=4,
            n_scale=1,
    ...
    def forward(self,
        ...
        # (1) raw sparse depth, (2) filtered validity mapprint
        imgs_of_kbnet_dirpath = os.path.join(output_path, 'imgs_of_kbnet', str(idx).zfill(10))
        if not os.path.exists(imgs_of_kbnet_dirpath):
            os.makedirs(imgs_of_kbnet_dirpath)
        ...
        input_normal = self.sparse_to_dense_pool_normal(input_depth, normal, intrinsics)
        input_depth = self.sparse_to_dense_pool(input_depth)

        # Forward through the network  skips:list  latents:tensor
        latent, skips = self.encoder(image, input_depth, input_normal, intrinsics)
        output_0 = self.decoder(latent, skips)

        output = output_0[-1]
        output_depth = output[:, 0, :, :].unsqueeze(1)
        output_normal = output[:, 1:, :, :]
        output_depth = torch.sigmoid(output_depth)
        output_normal = torch.sigmoid(output_normal)

        output_depth = \
            self.min_predict_depth / (output_depth + self.min_predict_depth / self.max_predict_depth)

        print('output', output.size(), output_depth.size(), output_normal.size())
    ...
    def compute_loss(self,
        ...
        l = w_{ph}l_{ph} + w_{sz}l_{sz} + w_{sm}l_{sm} + w_{nor}l_{nor}
        ...
        Arg(s):
            ...
            output_normal: torch.Tensor[float32]
                N x 3 x H x W output depth at time t
        ...
        points_3dim, points = net_utils.backproject_to_camera(output_depth0, intrinsics, shape)
        # points_normal, normal = net_utils.backproject_to_camera_normal(output_normal, intrinsics)

        # Reproject points onto image 1 and image 2
        # depth投到image1/image2坐标系下的坐标（u，v）
        ...
        # 把image1/iamge2 通过双线性插值投影到image0
        ...
        loss_normal = losses.normal_loss_func(
            image0,
            output_depth0,
            output_normal,
            intrinsics
        ...
        # l = w_{ph}l_{ph} + w_{sz}l_{sz} + w_{sm}l_{sm} +normal
        w_normal = 0.02
    ...
    def parameters(self):
        ...
        parameters = \
            ...
            list(self.sparse_to_dense_pool_normal.parameters()) + \
    ...
    def train(self):
        ...
        self.sparse_to_dense_pool_normal.train()
        self.encoder.train()
        self.decoder.train()
        ...
        保证BN层能够用全部训练数据的均值和方差，即测试过程中要保证BN层的均值和方差不变。对于Dropout，model.eval()是利用到了所有网络连接，即不进行随机舍弃神经元。
        '''
    ...
    def eval(self):
        ...
        self.sparse_to_dense_pool_normal.eval()
        self.encoder.eval()
        self.decoder.eval()
    ...
    def to(self, device):
        ...
        self.sparse_to_dense_pool_normal.to(device)
    ...
    def save_model(self, checkpoint_path, step, optimizer):
        ...
        checkpoint['sparse_to_dense_pool_normal_state_dict'] = self.sparse_to_dense_pool_normal.state_dict()
    ...
    def restore_model(self, checkpoint_path, optimizer=None):
        ...
        self.sparse_to_dense_pool_normal.load_state_dict(checkpoint['sparse_to_dense_pool_normal_state_dict'])
    ...
    def data_parallel(self):
        ...
        self.sparse_to_dense_pool_normal = torch.nn.DataParallel(self.sparse_to_dense_pool_normal)
