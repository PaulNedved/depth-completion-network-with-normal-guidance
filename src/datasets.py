...
def load_normal(normal_path):
    normal = data_utils.load_normal(normal_path)
    return normal

def load_test_normal(normal_path):
    normal = data_utils.load_test_normal(normal_path)
    return normal
...
class KBNetTrainingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        normal0 = load_normal(self.normal_paths[index])
        ...
        # Convert to float32
        image0, image1, image2, sparse_depth0, normal0, intrinsics = [
            T.astype(np.float32)
            for T in [image0, image1, image2, sparse_depth0, normal0, intrinsics]
        ]

        return image0, image1, image2, sparse_depth0, normal0, intrinsics
...
class KBNetInferenceDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load normal
        normal = load_test_normal(self.normal_paths[index])
        ...
        return image, sparse_depth, normal, intrinsics
