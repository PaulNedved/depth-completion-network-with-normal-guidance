...
'''
print
KITTI_RAW_DATA_DIRPATH = os.path.join('data0', 'kitti_raw_data')
print('KITTI_RAW_DATA_DIRPATH:中！', KITTI_RAW_DATA_DIRPATH)
KITTI_DEPTH_COMPLETION_DIRPATH = os.path.join('data0', 'kitti_depth_completion')
print('KITTI_DEPTH_COMPLETION_DIRPATH:', KITTI_DEPTH_COMPLETION_DIRPATH)
KITTI_TRAINVAL_SPARSE_DEPTH_DIRPATH = os.path.join(
    KITTI_DEPTH_COMPLETION_DIRPATH, 'train_val_split', 'sparse_depth')
print('KITTI_TRAINVAL_SPARSE_DEPTH_DIRPATH:', KITTI_TRAINVAL_SPARSE_DEPTH_DIRPATH)
...
KITTI_DEPTH_COMPLETION_OUTPUT_DIRPATH = os.path.join(
    'data0', 'kitti_depth_completion_kbnet')

TRAIN_OUTPUT_REF_DIRPATH = os.path.join('training0', 'kitti')
VAL_OUTPUT_REF_DIRPATH = os.path.join('validation0', 'kitti')
TEST_OUTPUT_REF_DIRPATH = os.path.join('testing0', 'kitti')
...
for refdir in ['train', 'val']:
    ...
    for sequence_dirpath in sparse_depth_sequence_dirpath:
        ...
        for camera_dirpath in ['image_02', 'image_03']:
            ...
            # sparse_depth_paths路径无输出/有输出??? 且 279报错?  有个循环错了或结束语句有误
            #print('###########', sparse_depth_paths, '\n')
            ...
            #print('##281### Load sparse depth and save validity map')
            #print('索引范围：', sparse_depth_paths)


            #这个for循环进了
            for idx in range(len(sparse_depth_paths)):
                # ok! print('sparse_depth_paths目录不为空')
                ...
                # ok! print('##292##image0_path###', image0_path)
                image0_path_idx = image_paths.index(image0_path)
                # ok! print('##294##image0_path_idx###', image0_path_idx)
            ...
            with mp.Pool(args.n_thread) as pool:
                print('###309###get into mp.Pool(args.n_thread)')
...
# ok print('##346###train_image_output_paths###',train_image_output_paths, 'n_sample number:', n_sample)
...
#print('####390###Storing training image file paths into', train_image_output_paths, '\n')
#print('####391###train_image_output_paths is :', train_image_output_paths)
...
print('##448##,clean output path:', train_image_clean_output_paths)
...
#('####594#####', VAL_IMAGE_OUTPUT_FILEPATH, val_image_output_paths)
